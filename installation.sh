...
cd ${installationDirectory}
git clone https://github.com/lh3/bwa.git		#cherche la totalité du code et l'historique de bwa. Ainsi, si une maj à été faite hier, on l'a aujourd'hui. On est sûr d'avoir la dernière version

# Create the executable
cd bwa
make		#compile le script pour créer un exécutable à partir de tous les fichiers différents

# Export the path of the executable (such that bwa can be lunched from anywhere)
echo 'export PATH='$(pwd)':${PATH}' >> ~/.bashrc	#dit à l'interface de chercher le fichier bwa que dans le dossier courant (pwd)
...
tar -xjf gatk-3.8-1.tar.bz2	#décompresser
